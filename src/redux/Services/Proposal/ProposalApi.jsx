...
export const GetSelectedServicePackageAccept = async (quoteKeyID, servicePackageKeyID, Action, ServiceChargeTypeID, ContractSignatoryKeyID) => {
...
export const GetQuoteContractViewPDFurl = async (UserKeyID, OrganisationKeyID, ModuleName, ModuleKeyID) => {
...
export const GetAcceptedQuotationServiceDetails = async (params) => {
    ...
    const res = await postApiWithAuthenticated(
        ProposalBaseUrlQuote + url, params
...
export const GetQuoteLookupList = async (organisationKeyId) => {
    const res = await getListWithAuthenticated(ProposalBaseUrlQuote + `/GetQuoteLookupList?OrganisationKeyID=${organisationKeyId}`);
    return res;
};
